...
class EnvmapMaterialNetwork(nn.Module):
    def __init__(
        ...
        if self.white_light:
            ...
            self.lgtSGs = nn.Parameter(
                torch.randn(self.numLgtSGs, 5), requires_grad=True
            )  # [M, 5]; lobe + lambda + mu
            # self.lgtSGs.data[:, -1] = torch.clamp(torch.abs(self.lgtSGs.data[:, -1]), max=0.01)
        else:
            self.lgtSGs = nn.Parameter(
                torch.randn(self.numLgtSGs, 7), requires_grad=True
            )  # [M, 7]; lobe + lambda + mu
        ...
        self.lgtSGs.data[:, 3:4] = 20.0 + torch.abs(self.lgtSGs.data[:, 3:4] * 100.0)
        ...
        if self.fix_specular_albedo:
            ...
            assert np.all(np.logical_and(specular_albedo > 0.0, specular_albedo < 1.0))
            self.specular_reflectance = nn.Parameter(
                torch.from_numpy(specular_albedo).reshape((self.numBrdfSGs, 3)),
                requires_grad=False,
            )  # [K, 1]
        else:
            if self.white_specular:
                ...
                self.specular_reflectance = nn.Parameter(
                    torch.randn(self.numBrdfSGs, 1), requires_grad=True
                )  # [K, 1]
            else:
                self.specular_reflectance = nn.Parameter(
                    torch.randn(self.numBrdfSGs, 3), requires_grad=True
                )  # [K, 3]
        ...
        roughness = [
            np.random.uniform(1.5, 2.0) for i in range(self.numBrdfSGs)
        ]  # big roughness
        roughness = (
            np.array(roughness).astype(dtype=np.float32).reshape((self.numBrdfSGs, 1))
        )  # [K, 1]
        print("init roughness: ", 1.0 / (1.0 + np.exp(-roughness)))
        self.roughness = nn.Parameter(torch.from_numpy(roughness), requires_grad=True)
        ...
        if self.numBrdfSGs > 1:
            dim = input_dim
            for i in range(3):
                self.blending_weights_layers.append(
                    nn.Sequential(nn.Linear(dim, 256), self.actv_fn)
                )
                dim = 256
    ...
    def load_light(self, path):
        ...
        self.lgtSGs = nn.Parameter(
            torch.from_numpy(np.load(path)).to(device), requires_grad=True
        )
    ...
    def get_base_materials(self):
        ...
        if self.fix_specular_albedo:
            ...
            specular_reflectacne = torch.sigmoid(
                self.specular_reflectance.clone().detach()
            )
            if self.white_specular:
    ...
    def forward(self, points):
        if points is None:
            ...
            if self.numBrdfSGs > 1:
                blending_weights = F.softmax(
                    self.blending_weights_layers(points), dim=-1
                )
            else:
                blending_weights = None
