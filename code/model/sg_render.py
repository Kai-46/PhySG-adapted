...
def compute_envmap_pcd(lgtSGs, N=1000, upper_hemi=False):
    ...
    if upper_hemi:
        # y > 0
        viewdirs = torch.cat(
            (viewdirs[:, 0:1], torch.abs(viewdirs[:, 1:2]), viewdirs[:, 2:3]), dim=-1
        )
...
def render_with_sg(
    ...
    normal = (
        normal.unsqueeze(-2).unsqueeze(-2).expand(dots_shape + [M, K, 3])
    )  # [..., M, K, 3]

    # viewdirs = viewdirs / (torch.norm(viewdirs, dim=-1, keepdim=True) + TINY_NUMBER)  # [..., 3]; ---> camera
    viewdirs = (
        viewdirs.unsqueeze(-2).unsqueeze(-2).expand(dots_shape + [M, K, 3])
    )  # [..., M, K, 3]
    ...
    inv_roughness_pow4 = 1.0 / (roughness * roughness * roughness * roughness)  # [K, 1]
    brdfSGLambdas = prepend_dims(
        2.0 * inv_roughness_pow4,
        dots_shape
        + [
            M,
        ],
    ...
    ### note: for numeric stability
    v_dot_lobe = torch.clamp(v_dot_lobe, min=0.0)
    warpBrdfSGLobes = 2 * v_dot_lobe * brdfSGLobes - viewdirs
    warpBrdfSGLobes = warpBrdfSGLobes / (
        torch.norm(warpBrdfSGLobes, dim=-1, keepdim=True) + TINY_NUMBER
    )
    ...
    ### note: for numeric stability
    v_dot_h = torch.clamp(v_dot_h, min=0.0)
    specular_reflectance = prepend_dims(
        specular_reflectance,
        dots_shape
        + [
            M,
    ...
    if blending_weights is None:
        ...
        specular_rgb = (specular_rgb.sum(dim=-3) * blending_weights.unsqueeze(-1)).sum(
            dim=-2
        )
    specular_rgb = torch.clamp(specular_rgb, min=0.0)
    ...
    if diffuse_rgb is None:
        diffuse = (
            (diffuse_albedo / np.pi)
            .unsqueeze(-2)
            .unsqueeze(-2)
            .expand(dots_shape + [M, 1, 3])
        )

        # multiply with light sg
        final_lobes = lgtSGLobes.narrow(
            dim=-2, start=0, length=1
        )  # [..., M, K, 3] --> [..., M, 1, 3]
        ...
        diffuse_rgb = torch.clamp(diffuse_rgb, min=0.0)
