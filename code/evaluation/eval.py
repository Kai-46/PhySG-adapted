...
from utils import rend_util, vis_util
...
def evaluate(**kwargs):
    ...
    evaldir = os.path.join(
        "../", evals_folder_name, expname, os.path.basename(kwargs["data_split_dir"])
    )

    model = utils.get_class(conf.get_string("train.model_class"))(
        conf=conf.get_config("model")
    )
    ...
    eval_dataset = utils.get_class(conf.get_string("train.dataset_class"))(
        ...
        train_cameras=False,
        eval=True,
        eval_idx=int(kwargs["light_sg"].split("/")[-2].replace("gt_env_", "")),
    )
    ...
    if kwargs["light_sg"].endswith(".npy"):
        ...
        evaldir = evaldir + "_relight_{}".format(kwargs["light_sg"].split("/")[-2])
        relight = True
    ...
    with open(os.path.join(evaldir, "ckpt_path.txt"), "w") as fp:
        fp.write(ckpt_path + "\n")
    shutil.copy2(ckpt_path, os.path.join(evaldir, "ckpt.pth"))
    ...
    for data_index, (indices, model_input, ground_truth) in enumerate(eval_dataloader):
        ...
        if edit_diffuse:
            diffuse_albedo = imageio.imread(kwargs["diffuse_albedo"]).astype(
                np.float32
            )[:, :, :3]
            if not kwargs["diffuse_albedo"].endswith(".exr"):
                diffuse_albedo /= 255.0
            diffuse_albedo = torch.from_numpy(diffuse_albedo).cuda().reshape((-1, 3))

            ray_dirs, _ = rend_util.get_camera_params(
                model_input["uv"], model_input["pose"], model_input["intrinsics"]
            )
            sg_ret = model.render_sg_rgb(
        ...
        tonemap_img = lambda x: np.power(x, 1.0 / eval_dataset.gamma)
        clip_img = lambda x: np.clip(x, 0.0, 1.0)
        ...
        if kwargs["save_exr"]:
            imageio.imwrite(
                "{0}/sg_rgb_{1}.exr".format(images_dir, out_img_name), rgb_eval
            )
        ...
        if kwargs["save_exr"]:
            imageio.imwrite(
                "{0}/normal_{1}.exr".format(images_dir, out_img_name), normal
            )
        ...
        if (not relight) and eval_dataset.has_groundtruth:
            ...
            network_object_mask = (
                model_outputs["network_object_mask"] & model_outputs["object_mask"]
            )
            depth_valid = rend_util.get_depth(
                model_outputs["points"].reshape(batch_size, total_pixels, 3),
                model_input["pose"],
            ).reshape(-1)[network_object_mask]
            depth[network_object_mask] = depth_valid
            depth[~network_object_mask] = 0.98 * depth_valid.min()
            assert batch_size == 1
            network_object_mask = (
                network_object_mask.float().reshape(img_res[0], img_res[1]).cpu()
            )
            ...
            if kwargs["save_exr"]:
                ...
                depth = depth.numpy()
                imageio.imwrite(
                    "{0}/depth_{1}.exr".format(images_dir, out_img_name), depth
                )
                ...
                depth = depth * network_object_mask.unsqueeze(-1) + (
                    1.0 - network_object_mask.unsqueeze(-1)
                )
                depth = depth.numpy()
            ...
            envmap = compute_envmap(
                lgtSGs=model.envmap_material_network.get_light(),
                H=256,
                W=512,
                upper_hemi=model.envmap_material_network.upper_hemi,
            )
            ...
            ) = model.envmap_material_network.get_base_materials()
            ...
            if kwargs["save_exr"]:
                imageio.imwrite(
                    "{0}/gt_{1}.exr".format(images_dir, out_img_name), rgb_gt
                )
            ...
            if kwargs["save_exr"]:
                imageio.imwrite(
                    "{0}/sg_diffuse_albedo_{1}.exr".format(images_dir, out_img_name),
                    rgb_eval,
                )
                ...
                img = Image.fromarray((rgb_eval * 255).astype(np.uint8))
                img.save(
                    "{0}/sg_diffuse_albedo_{1}.png".format(images_dir, out_img_name)
                )
            ...
            if kwargs["save_exr"]:
                imageio.imwrite(
                    "{0}/sg_diffuse_rgb_{1}.exr".format(images_dir, out_img_name),
                    rgb_eval,
                )
            ...
            if kwargs["save_exr"]:
                imageio.imwrite(
                    "{0}/sg_specular_rgb_{1}.exr".format(images_dir, out_img_name),
                    rgb_eval,
                )
    ...
    if not kwargs["save_exr"]:
        imageio.mimwrite(
            os.path.join(images_dir, "video_rgb.mp4"), all_frames, fps=15, quality=9
        )
    ...
    if len(psnrs) > 0:
        ...
        print(
            "RENDERING EVALUATION: psnr mean = {0} ; psnr std = {1}".format(
                "%.2f" % psnrs.mean(), "%.2f" % psnrs.std()
            )
        )
...
def get_cameras_accuracy(
    ...
    Q = torch.addbmm(
        torch.zeros(d, d, dtype=torch.double), gt_Rs, pred_Rs.transpose(1, 2)
    )
    Uq, _, Vq = torch.svd(Q)
    ...
    obj = cp.Minimize(
        cp.sum(
            cp.norm(
                gt_ts.numpy()
                - (c_opt * pred_ts.numpy() + np.ones((n, 1), dtype=np.double) @ t_opt),
                axis=1,
            )
        )
    )
    ...
    prob.solve()
    t_fixed = (
        c_opt.value * pred_ts.numpy() + np.ones((n, 1), dtype=np.double) * t_opt.value
    )
    ...
    print(
        "CAMERAS EVALUATION: R error mean = {0} ; t error mean = {1} ; R error median = {2} ; t error median = {3}".format(
            "%.2f" % R_error_mean,
            "%.2f" % t_error_mean,
            "%.2f" % R_error_medi,
            "%.2f" % t_error_medi,
...
def compare_rotations(R1, R2):
    cos_err = (
        torch.bmm(R1, R2.transpose(1, 2))[:, torch.arange(3), torch.arange(3)].sum(
            dim=-1
        )
        - 1
    ) / 2
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gamma", type=float, default=1.0, help="gamma correction coefficient"
    )
    ...
    parser.add_argument(
        "--expname", type=str, default="", help="The experiment name to be evaluated."
    )
    parser.add_argument(
        "--exps_folder", type=str, default="exps", help="The experiments folder name."
    )
    parser.add_argument(
        "--timestamp",
        default="latest",
    ...
    parser.add_argument(
        "--resolution", default=512, type=int, help="Grid resolution for marching cube"
    )
    parser.add_argument(
        "--is_uniform_grid",
        default=False,
        action="store_true",
        help="If set, evaluate marching cube with uniform grid.",
    )
